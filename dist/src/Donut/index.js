...
var InfinityDonut = /*#__PURE__*/function (_React$Component) {
  ...
  function InfinityDonut(props) {
    ...
    _this.changeTitle = _this.changeTitle.bind(_assertThisInitialized(_this));
    _this.changeOthers = _this.changeOthers.bind(_assertThisInitialized(_this));
    return _this;
  }
  ...
  _createClass(InfinityDonut, [{
    ...
    value: function calculateValues() {
      ...
      for (var i = 0; i < keys.length; i++) {
        if (coins_b[keys[i]] != undefined) {
          this.total_balance += parseFloat(coins_b[keys[i]].amountUSD);
        }
      }
      ...
      if (this.total_balance > 0) {
        for (var i = 0; i < keys.length; i++) {
          var bal = parseFloat(coins_b[keys[i]].amountUSD);
    ...
    key: "changeOthers",
    value: function changeOthers(tt) {
      var ts = this;
      this.setState({
        others: tt
      }, function () {
    ...
    value: function updateCanvas() {
      ...
      if (!this.hovering) {
        ...
        if (this.coin != "others") {
          ...
          var bal = this.state.currency.symbol + preparePrice(this.balances[this.coin] * this.state.currency.rate, this.state.currency.decimals);
          var measure_text = context.measureText(bal);
          context.fillText(bal, this.state.width / 2 - measure_text.width * 0.5, this.state.width / 2 + aux_aux_size * 0.5 + size_aux * 0.5 + 10 - 2);
          context.closePath();
        }
